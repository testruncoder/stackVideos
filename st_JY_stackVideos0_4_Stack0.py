...
def stack_videos(_clips,finalVideo_width,DEFAULT_TEMPFILE):
       ...
       timeStampStr=time_stamp_string()
       final_filename='stacked_'+timeStampStr+'.mp4'
       time.sleep(1)
...
#     # ----------------------------------------------------------------------------------------------------
       ...
       with st.sidebar:
              ...
              with st.expander('Current Folder:',expanded=True):
                    st.markdown(f'Folder for Video #1:   \n{folder_path1}')
                    st.markdown(f'Folder for Video #2:   \n{folder_path2}')
       ...
       if not two_vdos_selected and not two_vdos_chkbox:
              ...
              if stack_videos_button:
                     with st.spinner('Getting video files ready for stacking...'):
                            clip1=VideoFileClip(vdo1)
                            clip2=VideoFileClip(vdo2)
