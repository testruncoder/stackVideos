...
#     # ----------------------------------------------------------------------------------------------------
       ...
       with st.sidebar:
              vdo=st.file_uploader('Upload two video files', type=vdo_formats, 
              ...
              folder_path1=st.text_input('Enter a folder path for video',
                                         ...
                                         help='Please place an input video file in the current folder.',
       ...
       with st.form('Clip Video'):
              _,col1,_=st.columns((2,4,2))
              st.markdown('### Video Clip')
              c1,_,c2=st.columns((5,1,5))
              startTime_vdo1=c1.number_input('Start time:', min_value=0.0, 
                                                 value=0.0, key='subc1start_numinput')
              endTime_vdo1=c2.number_input('End time:', min_value=0.0, 
                                                 value=3.0, key='subc1end_numinput')
              always_clip=c2.checkbox('Always Clip',value=True,label_visibility='visible', 
                          help='If checked, clip a video even when you refresh the app page.',
                          key='clip_setting')
              c1.form_submit_button('Apply settings', on_click=_change_callback)
       ...
       if state.is_modified and always_clip:
              if vdo is not None:
                     ...
                     if state.clip_setting:
                            ...
                            with st.spinner('Getting ready to show clipped video file #1...'):
                                                                             ...
                                                                             # time.sleep(1)
                                                                             # os.rename(DEFAULT_TEMPFILE,clip1_filename)
                                   vdo_name=vdo.name.split('.')[0] + '.mp4'
                                   final_clip_filename=save_clip1(clip1,vdo_name,finalVideo_width,DEFAULT_TEMPFILE)
                                   final_clip_file_w_path=os.path.join(curr_dir, final_clip_filename)
                                   time.sleep(1)
                                   st.success(f'Successfully saved in    \n'+f'{final_clip_file_w_path}')

                                   # # Create a downlink of the clip1 video file;
                                   with c1:
                                          # st.markdown('Download Link for Clipped Video:')
                                          create_download_link(final_clip_file_w_path)
                            # msg_empty.info('Clipped video successfully')
                      
                            st.subheader('Play Video')
                            # # Option 1: Play a video by st.video()
                            # with st.expander('Expand to view the clipped video',expanded=False):
                            _,col1,_=st.columns((1,8,1))
                            col1.video(final_clip_filename)
                            col1.caption(final_clip_filename)
                            st.markdown('')
