...
#     # ----------------------------------------------------------------------------------------------------
       ...
       def _change_callback():
              ...
              # settings=[state.frameIndex_setting]
       ...
       with st.sidebar:
              vdo=st.file_uploader('Upload a video file', type=vdo_formats, 
                                   accept_multiple_files=False,)
              if vdos is None:
                     st.warning('To start, choose a video file.') 
              tmp_info=st.empty()
              tmp_info.info('Place an input video file in the current folder as shown below.')
              if vdo is not None:
                    tmp_info.empty()
              folder_path1=st.text_input('Enter a folder path for video',
                                         value=curr_dir,  # media_dir,
                                         help='Please place an input video file in the current path.',
                                         key='folderpath1_textinput')
              with st.expander('Current Folder:',expanded=True):
                    st.markdown(f'{curr_dir}')

       if vdo is not None:          
              # vdo1=os.path.join(folder_path1,vdo.name)
              vdo1=os.path.join(curr_dir,vdo.name)

              # # Gather fps and frame size
              _clip1=VideoFileClip(vdo1)
              vdo_size=_clip1.size
              vdo_fps=_clip1.fps

              # # Find the total number of frames per video file
              num_frames=len(list(_clip1.iter_frames()))

              with st.sidebar.expander('Selected images',expanded=True):
                     st.video(vdo1)
                     st.caption(f'#1: {vdo.name}')
                     st.caption(f'size: {_clip1.size}, fps: {round(_clip1.fps)}, length: {_clip1.duration:.2f} sec')
                     _clip1.close()
              with st.sidebar.expander('Selected path and file',expanded=False):
       ...
       if vdo is not None:
              ...
              if cl1.button('Insert Frame Numbers'):  # 'Play/Save Video':
                     time.sleep(0.5)
                     tmp_info=st.empty()
                     _,col1,_=st.columns((1,9,1))
                     # st.button('Clear Video Player',key='clearvideoplayer455)
                     # if state.frame_numbers_setting:
                     tmp_info.info('Play and then save the video with frame numbers displayed...')

                     frameNum = 0
                     video = cv2.VideoCapture(vdo1)
                     video.set(cv2.CAP_PROP_FPS,25)

                     fontSize=cv2_putText_fontSize(vdo_size)
                     fontCoord=cv2_putText_fontCoord(vdo_size)
                     image_placeholder=st.empty()
                     prog_placeholder=st.empty()

                     # # Create an output video file name
                                                               # timeStampStr=time_stamp_string()
                                                               # vfileName1='frameNumbered_'+vdo1
                     vfileName1='frameNumbered_'+vdo.name.split('.')[0]+'.mp4'
                     vfileName1=os.path.join(curr_dir,vfileName1)
                     fourcc=cv2.VideoWriter_fourcc(*'avc1')  #  "MP4V" does not play in web browswers.
                                                 # 'acv1' requires 'openh264-1.8.0-win64.dll' to be placed in the same folder as this python code is.
                     out1=cv2.VideoWriter(vfileName1,fourcc,vdo_fps,vdo_size)  # clip1 
                     
                     prog_placeholder.progress(0.0)
                     while True and not stop_play:
                            frameNum += 1
                            success,image=video.read()
                            if not success:
                            ...
                            cv2.putText(image, f'# {str(frameNum)}', fontCoord, cv2.FONT_HERSHEY_COMPLEX,
                                          fontSize,(0,255,0),2)
                            out1.write(image)
                            image_placeholder.image(image,channels='BGR',caption=vdo.name)  # openCV has image in BGR
                            percent_complete=frameNum/num_frames
                            prog_placeholder.progress(percent_complete)
                            time.sleep(play_speed)  # Default play_speed=0.01
                     ...
                     col1.video(vfileName1)
                     col1.caption(vfileName1)
              ...
              # ------------------------- END OF if cl1.button('Play/Save Video')-------------------------------------

              if st.button('Refresh Page'):
                     pass
       else:  # vdo is None;
              st.warning('To start, choose a video file.')
# ------------------------------------------------------ END OF frameIndex_page() ----------------------------------------------------------
